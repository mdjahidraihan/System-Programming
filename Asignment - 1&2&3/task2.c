...
int main()
{
	...
	int bid=0;//number of background process running;
	...
	if((strcmp(c, "cd") == 0))
	{
 	...
 	if(strcmp(str,"&")==0)// for background process
    {
    	...
    	if(pid==0)
   		{
			...
			double Time = (double)usage.ru_utime.tv_sec*1000.0 + (double)usage.ru_utime.tv_usec/1000.0;

        	printf("User time consumed : %f ms\n",Time);

        	Time = (double)usage.ru_stime.tv_sec*1000.0+ (double)usage.ru_stime.tv_usec/1000.0;
